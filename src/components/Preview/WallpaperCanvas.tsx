...
export function WallpaperCanvas({
  ...
  // Aim for ~28px visually on the preview (after container scale is applied)
  const helperPreviewTargetPx = 28
  const helperBaseFontSizePx = Math.round(helperPreviewTargetPx / scale)
  const helperFontSizePx = Math.max(1, Math.round(helperBaseFontSizePx * 0.65))
  const helperFontWeight = Math.min(900, Math.round((typography.fontWeight * 1.3) / 100) * 100)
  const helperLetterSpacing = typography.letterSpacing * 0.75
    ...
    const generateQRs = async () => {
      ...
      for (const block of qrBlocks) {
        try {
          ...
          console.error(`Failed to generate QR for ${block.id}:`, error)
        }
      }
  ...
  const getGradientStyle = () => {
    if (!gradient) return { background: '#667eea' }

    const gradData = gradients.find((g) => g.id === gradient.id)
    if (!gradData) return { background: '#667eea' }

    if (gradData.type === 'linear') {
      return {
        background: `linear-gradient(${gradData.angle || 135}deg, ${gradData.colors.join(', ')})`,
      }
    } else {
      return {
        background: `radial-gradient(circle, ${gradData.colors.join(', ')})`,
      }
    }
  }

  const getFontFamily = () => {
    const font = fonts.find((f) => f.id === typography.fontFamily)
  ...
  return (
    ...
    >
      ...
      {/* Safe area overlay (optional, can be toggled) */}
      {device && (
        <div
          className="absolute border-2 border-dashed border-white/30 pointer-events-none"
          style={{
            top: `${device.safeArea.top}px`,
            right: `${device.safeArea.right}px`,
            bottom: `${device.safeArea.bottom}px`,
            left: `${device.safeArea.left}px`,
          }}
        />
      )}
            ...
            >
              {block.iconType && block.iconType !== 'custom' && (
                ...
                >
                  ...
                  {block.iconType === 'website' && (
                    <g fill="none" stroke={block.color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {qrBlocks.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="flex flex-col items-center text-center px-8">
            ...
            {/* Label matching actual typography */}
            <p 
              className="font-medium text-white/70"
              style={{
                fontSize: `${helperFontSizePx}px`,
                fontWeight: typography.fontWeight,
              }}
            >
